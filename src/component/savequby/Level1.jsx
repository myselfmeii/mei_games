...
import qubystanding from '../../../public/qubystanding.png';
...
const ParallaxBackground = ({ playerX }) => {
    // The background layers will move at different speeds based on the player's position.
...
// Player Component with new image
const Player = ({ x, y, isJumping, isMoving, isFalling, zIndex }) => {
    // This component renders the player character.
    // A rotation is added when the player is falling.
    ...
    // Use the new image for the player
    return (
...
// Platform Component
const Platform = ({ x, y, width, height, isActivated }) => {
    const platformClasses = `platform ${isActivated ? 'animate-trap-grow' : ''}`;
    const platformStyle = {
        left: x,
        top: y,
        width,
...
const Cave = ({ x, y, width, height }) => {
    ...
    return (
        <div className="cave" style={style}>
            <div className="cave-opening"></div>
        </div>
    );
};

// Game UI (Lives and Timer)
const GameUI = ({ lives, timer }) => {
    const minutes = Math.floor(timer / 60);
    const seconds = timer % 60;

    const getTimerColor = () => {
        if (timer <= 30) return '#EF4444'; // Red
        if (timer <= 60) return '#F97316'; // Orange
        return '#22C55E'; // Green
    };
...
const RoundIndicator = ({ roundNumber }) => {
    return (
        <div className="main">
            ...
            <span className="socod">{roundNumber}</span>
        </div>
    );
};
...
const Level1 = ({ onLevelComplete, onPlayerDeath, shouldReset }) => {
    ...
    useEffect(() => {
        ...
        if (deathTrigger.type === 'trap') {
            const timer = setTimeout(() => onPlayerDeath(), 300);
    ...
    const handleKeyDown = useCallback((e) => {
        switch (e.key) {
            ...
            case ' ': case 'ArrowUp': e.preventDefault(); setKeys(prev => ({ ...prev, up: true })); break;
    ...
    const handleKeyUp = useCallback((e) => {
        switch (e.key) {
            ...
            case ' ': case 'ArrowUp': setKeys(prev => ({ ...prev, up: false })); break;
    ...
    const gameLoop = useCallback(() => {
        ...
        setPlayer(p => {
            ...
            else if (keys.left) vx = -currentSpeed;
            else vx = 0;
            ...
            if (deathTrigger.active) {
                return { ...p, x: PLAYER_START_X, y: PLAYER_START_Y, vx: 0, vy: 0, isJumping: true };
            }

            if (!onPlatform && y < GAME_HEIGHT - PLAYER_HEIGHT) isJumping = true;
    ...
    }, [keys, onLevelComplete, platforms, cave, activatedTrapId, deathTrigger.active]);
    ...
    return (
        <div className="level-container">
            ...
            <Player x={player.x} y={player.y} isJumping={player.isJumping} isMoving={keys.left || keys.right} isFalling={player.vy > 1 && player.isJumping} zIndex={playerZIndex} />
...
export default function App() {
    ...
    const handleLevelComplete = () => setGameState('won');
        ...
        <div className="app-container">
            <style>
                {`
                    ...
                    .game-container { position: relative; background-color: #A5F3FC; }
            ...
            <div className="game-container" style={{ width: GAME_WIDTH, height: GAME_HEIGHT }}>
                {gameState === 'playing' && (
                    <>
                        <RoundIndicator roundNumber={INITIAL_LIVES - lives + 1} />
                        <GameUI lives={lives} timer={timer} />
                        <Level1 onLevelComplete={handleLevelComplete} onPlayerDeath={handlePlayerDeath} shouldReset={resetLevel} />
                    </>
                )}
                {gameState !== 'playing' && (
